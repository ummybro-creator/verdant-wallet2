...
async function sendTelegramNotification(
  ...
  if (!chatId || !botToken) {
    try {
      ...
      if (settingsErr) {
        console.error("[mpxpay-callback] Failed to read app_settings:", settingsErr.message);
      } else {
      ...
      console.error("[mpxpay-callback] Exception reading app_settings:", e);
    }
  }
  ...
  if (!botToken) {
    console.error(
      "[mpxpay-callback] ❌ Telegram bot token is not configured. " +
  ...
  if (!chatId) {
    console.error(
      "[mpxpay-callback] ❌ Telegram chat ID is not configured. " +
