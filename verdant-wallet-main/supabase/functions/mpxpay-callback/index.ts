// Supabase Edge Function: mpxpay-callback
// Receives MPX Pay POST callbacks when a payment completes.
...
async function resolveChatId(serviceClient: any): Promise<string | null> {
  ...
  try {
    ...
    if (cached) {
      console.log(`[mpxpay-callback] Using cached chat_id: ${cached}`);
      return cached;
    }
  } catch (e) {
    console.warn("[mpxpay-callback] Could not read app_settings:", e);
  }
...
async function sendTelegramNotification(
  ...
  if (!chatId) {
    console.error(
      "[mpxpay-callback] ❌ Cannot send Telegram notification — no chat ID available.\n" +
  ...
  try {
    ...
    if (!res.ok || !body.ok) {
      ...
      if (body.description?.includes("parse") || body.error_code === 400) {
        ...
        if (plainBody.ok) {
          console.log(`[mpxpay-callback] ✅ Telegram notification sent (plain text) to chat_id=${chatId}`);
          return;
        }
      }
      console.error(
        `[mpxpay-callback] ❌ Telegram sendMessage failed (HTTP ${res.status}):`,
        JSON.stringify(body),
      );
    } else {
      console.log(`[mpxpay-callback] ✅ Telegram notification sent to chat_id=${chatId}`);
    }
  } catch (err) {
    console.error("[mpxpay-callback] ❌ Telegram sendMessage exception:", err);
  }
}
...
serve(async (req) => {
  ...
  console.log("[mpxpay-callback] Received payload:", JSON.stringify(body));

  const orderNo = body.platform_order_no || body.orderNo || body.order_no;
  const merchantOrder = body.merchant_order_no || body.merchantOrder;
  const status = String(body.status || "").toLowerCase();
  const amount = body.amount;

  if (status !== "success" && status !== "1" && status !== "paid") {
    console.log(`[mpxpay-callback] Ignoring status: ${status}`);
    return new Response(
      JSON.stringify({ status: "success", message: "Callback received (non-success status)" }),
      { status: 200, headers: { "Content-Type": "application/json" } },
    );
  }

  if (!merchantOrder || amount == null) {
    console.error("[mpxpay-callback] Missing required fields:", body);
  ...
  if (prErr || !pr) {
    console.error("[mpxpay-callback] Payment request not found:", merchantOrder, prErr);
    return new Response("Order not found", { status: 404 });
  }

  if (pr.status === "completed") {
    console.log("[mpxpay-callback] Already processed:", merchantOrder);
    return new Response(
      JSON.stringify({ status: "success", message: "Callback received" }),
      { status: 200, headers: { "Content-Type": "application/json" } },
    );
  }
  ...
  const { error: balErr } = await serviceClient.rpc("admin_credit_wallet", {
    ...
    p_note: `MPX Pay deposit — order ${orderNo || merchantOrder}`,
  });

  if (balErr) {
    console.warn("[mpxpay-callback] admin_credit_wallet RPC failed, applying fallback:", balErr);
    await serviceClient.from("deposits").insert({
      user_id: pr.user_id,
      amount: expectedAmount,
      utr: orderNo || merchantOrder,
      status: "approved",
      admin_note: "Auto-credited by MPX Pay",
    });
    ...
    await serviceClient.from("transactions").insert({
      ...
      amount: expectedAmount,
      status: "success",
      note: `MPX Pay deposit — order ${orderNo || merchantOrder}`,
    });
  }
  ...
  await serviceClient.from("notifications").insert({
    ...
    body: `₹${expectedAmount.toFixed(2)} has been credited to your wallet via MPX Pay.`,
  });
  ...
  console.log(`[mpxpay-callback] Successfully credited ₹${expectedAmount} to user ${pr.user_id}`);
