  ...
  try {
    ...
    if (chatIds.length === 0) {
      try {
        ...
        console.warn("[mpxpay-callback] Failed to get telegram updates:", e);
      }
    }

    if (chatIds.length === 0) {
      console.warn("[mpxpay-callback] No telegram chat_id found to send message");
      return;
    }
    ...
    console.error("[mpxpay-callback] Telegram notification error:", err);
  }
}
